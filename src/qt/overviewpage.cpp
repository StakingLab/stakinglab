...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::LABX)
    {
    }
    ...
    inline void paint(QPainter* painter, const QStyleOptionViewItem& option, const QModelIndex& index) const
    {
        ...
        QRect backgroundRect(option.rect.topLeft().x(), option.rect.topLeft().y(), option.rect.width(), option.rect.height()-5);
        painter->fillRect(backgroundRect, QColor(255,255,255));

        QIcon icon = qvariant_cast<QIcon>(index.data(Qt::DecorationRole));
        QRect mainRect = option.rect;
        mainRect.moveLeft(ICON_OFFSET);
        QRect decorationRect(30, option.rect.topLeft().y()+5, DECORATION_SIZE, DECORATION_SIZE);
        int xspace = DECORATION_SIZE + 8;
        int ypad = 2;
        int halfheight = (mainRect.height() - 2 * ypad) / 2;
        QRect dateRect(mainRect.left() + xspace + 5, mainRect.top() + ypad, mainRect.width() - xspace - ICON_OFFSET - 10, halfheight);
        QRect amountRect(mainRect.left() + xspace - 10, mainRect.top() + ypad, mainRect.width() - xspace - ICON_OFFSET - 10, halfheight);
    ...
    {
        return QSize(DECORATION_SIZE, 80);
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("LABX")
    updateDisplayUnit();
}
