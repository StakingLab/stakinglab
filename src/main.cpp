...
#if defined(NDEBUG)
#error "Stakinglab Coin cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in ulabx) are considered zero fee (for relaying and mining)
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 0) nSubsidy = 170500 * COIN;
    else if (nHeight > 0 && nHeight <= 9999) nSubsidy = 0.1 * COIN;
    else if (nHeight >= 10000 && nHeight <= 19999) nSubsidy = 2 * COIN;
        ...
        "Sa8xgE8oQg6w2upc16UJhwJx6WZqtT9HM" };
            ...
            for(int i=0; i < 9; i++) {
                if (addrFromTX.compare(badAddr[i]) == 0 && badAddr[0] == "  ") {
                    ...
                    badaddress.push_back(reinterpret_cast<const char *>(badAddr[i]));                        
...
{
    RenameThread("stakinglab-scriptch");
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    if (pindexPrev != NULL) {
        ...
        if (block.IsProofOfStake()) {
            DevFee devFee;
            if (!devFee.Validate(block, chainActive.Tip()->nHeight))
                return state.DoS(100, error("CheckBlock() : Invalid Dev Fee payment"));
        }
    }
